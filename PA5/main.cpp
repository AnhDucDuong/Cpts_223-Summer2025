...
void printHelp() {
    ...
    cout << " 2. listInventory <category_string> [desc|merge|merge desc]\n";
    cout << " 3. :quit\n\n";
}
...
void evalCommand(const string& line) {
    ...
    if (line.rfind("listInventory ", 0) == 0) {
        ...
        string category, option1, option2;
        iss >> category >> option1 >> option2;
        ...
        vector<InventoryItem*> items = listPtr->toVector();

        auto getPrice = [](InventoryItem* a) {
            try {
                return stod(a->sellingPrice);
            } catch (...) {
                return 0.0;
            }
        };

        auto ascending = [&](InventoryItem* a, InventoryItem* b) {
            return getPrice(a) < getPrice(b);
        };
        auto descending = [&](InventoryItem* a, InventoryItem* b) {
            return getPrice(a) > getPrice(b);
        };

        if (option1 == "merge") {
            if (option2 == "desc") mergeSort(items, descending);
            else mergeSort(items, ascending);
        } else {
            if (option1 == "desc") insertionSort(items, descending);
            else insertionSort(items, ascending);
        }

        for (auto* p : items) {
            cout << p->uniqId << " - " << p->productName << " ($" << p->sellingPrice << ")" << endl;
        }
    }
}
